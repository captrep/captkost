     ...
     if (!empty($_GET['err'])) :
      ...
      <script>
          setTimeout(function() {
              swal({
                  title: "Deleted!",
                  text: "Method berhasil dihapus!",
                  type: "success"
              }, function() {
                 window.location.href = "method_pembayaran.php"
              });
          }, 400);
      </script>
      <?php 
      ...
      <script>
          setTimeout(function() {
              swal({
                  title: "Oops!",
                  text: "Gagal menghapus method pembayaran",
                  type: "error"
              }, function() {
                window.location.href = "method_pembayaran.php"
              });
      ...
      <?php } endif ?>
        ...
        <div class="panel-body table-responsive">
          <table class="table table-hover">
            ...
            <tbody>
              <?php
                ...
                $tableMethod = mysqli_query($koneksi, "SELECT * FROM method");
                while($row=mysqli_fetch_array($tableMethod)) : ?>
                  <tr>
                    <td><?= $row['nama_method'];?></td>
                    <td><?= $row['payment'];?></td>
                    <div class="btn-group">
                      <td class="text-right">
                        <a href="edit_method.php?id_method=<?= $row['id_method'];?>" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                        <i id="confirm">
                        <a href="action/delete_method.php?id_method=<?= $row['id_method'];?>" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                      </i>
                      </td>
                    </div>
                  </tr>
          ...
          <a href="add_method.php" class="btn btn-primary margin-t-20 float-r">Tambah Method Pembayaran</a>
         </div>
      </div>
    </div>
  </div>
